...
void CButtonCommand::OnLoadMeshButtonClicked()
{
	...
	bool bSuccess = Platform->OpenFileDialog
	(
		Handle,
		"Open Dialog",
		DefaultPath,
		"",
		"Mesh Binary File|*.bin",
		EFileDialogFlags::None,
		FileNames
	);

	if (bSuccess == false || FileNames.Num() < 1)
	{
	...
	FVector End = Start + Client->GetViewRotation().RotateVector(FVector(1e+4f, 0.f, 0.f));
